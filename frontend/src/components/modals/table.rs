use crate::{
    components::{editor::EDITOR_ID, modals::utils::close_modal},
    contexts::{
...
pub fn table_modal() -> Html {
    let columns_state: UseStateHandle<Vec<Column>> =
        use_state_eq(|| vec![Column::from(Alignment::Center)]);
    ...
    let on_col_amount_change = Callback::from({
        let columns_state = columns_state.clone();
        move |_| {
            let col_textbox: HtmlInputElement = document()
                .get_element_by_id(&COLS_TEXTBOX_ID)
                .unwrap()
                .dyn_into()
                .unwrap();
    ...
    let on_row_amount_change = Callback::from({
        ...
        move |_| {
            let row_textbox: HtmlInputElement = document()
                .get_element_by_id(&ROW_TEXTBOX_ID)
                .unwrap()
                .dyn_into()
                .unwrap();
    ...
    let insert_table = Callback::from(move |_| {
        let link_title_input: HtmlParagraphElement = document()
            .get_element_by_id(&TABLE_PREVIEW_ID)
            .unwrap()
            .dyn_into()
            .unwrap();
