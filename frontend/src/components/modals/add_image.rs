use crate::components::{editor::textarea::textarea::EDITOR_ID, modals::utils::close_modal};
use crate::contexts::{
...
const PLACEHOLDER_LINK: AttrValue =
    AttrValue::Static("https://i.ytimg.com/vi/gvm-QXS3yjY/maxresdefault.jpg");
...
pub fn add_image_modal() -> Html {
    ...
    let insert_link = Callback::from(move |_| {
        let link_title_input: HtmlInputElement = document()
            .get_element_by_id(&TITLE_TEXTBOX_ID)
            .unwrap()
            .dyn_into()
            .unwrap();
        let link_title = link_title_input.value();

        let link_input: HtmlInputElement = document()
            .get_element_by_id(&LINK_TEXTBOX_ID)
            .unwrap()
            .dyn_into()
            .unwrap();
